...
(function () {
    ...
    angular
        .module('webuiApp')
        .controller('ScenarioDetailEditController', ScenarioDetailEditController);

    ScenarioDetailEditController.$inject = ['$scope', '$state', '$http', '$uibModalInstance', '$window'];

    function ScenarioDetailEditController($scope, $state, $http, $uibModalInstance, $window) {
        var vm = this;
        ...
        $http({
            ...
            $http({
                ...
                $http({
                    method: 'GET',
                    url: 'apiservice/api/scenario/' + $window.location.toString().split("/")[5] + "/apis"
                }).then(function successCallback(response) {
